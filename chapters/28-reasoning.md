...
Start
  ...
  |    └─ Yes → ReAct
  |
  ├─ Primarily arithmetic/code?
  |    └─ Yes → PAL (Program-Aided)
  |
  ├─ Need highest accuracy, cost no object?
  |    └─ Yes → Self-Consistency (n=20+) or Tree-of-Thought
  |
  ├─ Compositional/planning task?
  |    └─ Yes → Least-to-Most or Tree-of-Thought
  |
  ├─ Need backtracking/exploration?
  |    └─ Yes → Tree-of-Thought
  |
  ├─ Have process supervision data?
  |    └─ Yes → Train PRM + Best-of-N
  |
